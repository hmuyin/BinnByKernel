...
void* (*malloc_fn)(_In_ POOL_FLAGS Flags, _In_ SIZE_T NumberOfBytes, _In_ ULONG Tag) = 0;
...
#ifndef _MSC_VER
#define Rt_stricmp strcasecmp
#define Rt_strnicmp strncasecmp
#endif
...
void APIENTRY binn_set_alloc_functions(void* (*new_malloc)(_In_ POOL_FLAGS, _In_ SIZE_T, _In_ ULONG), void* (*new_realloc)(void*, size_t), void (*new_free)(void*)) {
...
BINN_PRIVATE void check_alloc_functions() {
	...
	if (malloc_fn == 0) malloc_fn = &ExAllocatePool2;
	if (realloc_fn == 0) realloc_fn = &ReallocateMemory;
	if (free_fn == 0) free_fn = &ExFreePool;
...
BINN_PRIVATE void* binn_malloc(int size) {
	...
	return malloc_fn(POOL_FLAG_NON_PAGED, size, 'BINN');
}
...
BINN_PRIVATE unsigned char* SearchForKey(unsigned char* p, int header_size, int size, int numitems, const char* key) {
	...
	for (i = 0; i < numitems; i++) {
		...
		if (len > 0) {
			if (Rt_strnicmp((char*)p, key, len) == 0) {   // note that there is no null terminator here
				if (keylen == len) {
					p += len;
					return p;
				}
...
BINN_PRIVATE BOOL binn_map_set_raw(binn* item, int id, int type, void* pvalue, int size) {
	...
	if (id <= 0x3F) {
		...
		*p++ = (UCHAR)(0xA0 | (sign << 4) | ((id & 0xF0000) >> 16));
		*p++ = (id & 0xFF00) >> 8;
		*p++ = id & 0xFF;
	}
	else if (id <= 0xFFFFFFF) {
		*p++ = 0xC0 | (sign << 4) | ((id & 0xF000000) >> 24);
		*p++ = (UCHAR)((id & 0xFF0000) >> 16);
		*p++ = (id & 0xFF00) >> 8;
		*p++ = id & 0xFF;
	}
	else {
...
BINN_PRIVATE BOOL AddValue(binn* item, int type, void* pvalue, int size) {
	...
	switch (storage_type) {
		...
		case BINN_STORAGE_STRING:
		if (size < 0) return FALSE;
		if (size == 0) size = (int)strlen2((char*)pvalue);
	...
	for (i = 0; i < count; i++) {
		...
		if ((*p & BINN_STORAGE_MASK) == BINN_STORAGE_CONTAINER) {
		  // recursively check the internal container
			int size2 = (int)(plimit - p + 1);  // maximum container size
...
BINN_PRIVATE BOOL copy_float_value(const void* psource, void* pdest, int source_type, int dest_type) {
	...
	UNREFERENCED_PARAMETER(dest_type);
...
binn* APIENTRY binn_value(int type, void* pvalue, int size, binn_mem_free freefn) {
	...
	if (item) {
		...
		switch (storage_type) {
			...
			break;
			case BINN_STORAGE_STRING:
			if (size == 0) size = (int)strlen((char*)pvalue) + 1;
			case BINN_STORAGE_BLOB:
			case BINN_STORAGE_CONTAINER:
...
BINN_PRIVATE BOOL is_bool_str(char* str, BOOL* pbool) {
	...
	if (Rt_stricmp(str, "true") == 0) goto loc_true;
	if (Rt_stricmp(str, "yes") == 0) goto loc_true;
	if (Rt_stricmp(str, "on") == 0) goto loc_true;
	//if (Rt_stricmp(str, "1") == 0) goto loc_true;

	if (Rt_stricmp(str, "false") == 0) goto loc_false;
	if (Rt_stricmp(str, "no") == 0) goto loc_false;
	if (Rt_stricmp(str, "off") == 0) goto loc_false;
	//if (Rt_stricmp(str, "0") == 0) goto loc_false;
...
loc_convert_value:
  ...
  //value->vint64 = 0;
	value->ptr = Rt_strdup(buf);
	if (value->ptr == NULL) return NULL;
	value->freefn = ExFreePool;
